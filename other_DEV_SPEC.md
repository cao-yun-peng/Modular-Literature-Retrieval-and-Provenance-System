...
####  开箱即用与深度扩展并重 (Plug-and-Play & Extensible)
...
## 7. 论文/文献专用适配（Paper-oriented RAG）

目标：将本项目从通用 RAG 适配为面向学术论文/技术文献的知识库，保证在摄取、切分、存储和检索各环节保留论文的结构化特征（题目/作者/摘要/章节/图表/参考文献），并在检索与响应构建时优先利用这些字段以提升可追溯性与准确性。

7.1 文档转换与解析
- 输入：PDF / XML / Markdown。
- 首选解析器：GROBID（paper-specific parsing: title/authors/refs/sections）、回退：MarkItDown/ PyMuPDF + 规则正则。
- 输出：标准化 Markdown（包含章节标题与图片占位符）+ JSON 元数据（`title, authors, year, venue, doi, page_ranges`）。

7.2 题目与作者抽取
- 优先使用解析器元数据（GROBID 输出）；回退规则：Markdown 文件头部首个 H1/H2 或前 5 行启发式匹配。
- 强制将 `title`/`authors` 写入每个 Document 的顶层 metadata，并继承到每个 chunk 的 metadata 中，便于按作者/年份过滤与排序。

7.3 摘要作为特殊 chunk
- 把 Abstract 单独作为 `chunk_type: abstract` 存储（单独 embedding，检索时赋予高权重）。
- 保存原始摘要与清洗版本（去除内嵌引用标注），并在检索命中时优先返回摘要作为回答依据。

7.4 分层切分策略（Chunking）
- 切分分层：
  - Document-level metadata（存储但不 embedding）
  - Abstract（单 chunk，`chunk_type=abstract`）
  - Section-level chunks（按章节标题分割）。若章节过长则按段落/句子语义切分以保证 tokens 上限。
  - Captions（图表/表格说明）作为独立 `chunk_type=caption`。
  - References（参考文献区）作为 `chunk_type=references`（默认降权或排除）。
- 参数建议：max_tokens 512-1024（或约 2–4 KB 文本），重叠 20%–30% 或 100–200 tokens，根据 embedding 模型调整。

7.5 参考文献处理
- 在解析时识别 `References` 区，尽量把每条参考文献解析为单独条目（author/title/venue/year/doi）。
- 存储策略：默认把整组参考文献作为一个低权重 chunk；可选地将每条引用解析为条目并写入引用表（索引 DOI → doc_id），但检索默认降权或仅在显式查询时启用。

7.6 元数据与存储 schema（示例 JSON）
下面是建议的 chunk metadata 样例，供向量库与检索过滤使用：

```json
{
  "chunk_id": "dochash_0001_a1b2c3d4",
  "doc_id": "dochash",
  "title": "An Example Paper on RAG",
  "authors": ["Zhang, Wei", "Li, Ming"],
  "year": 2024,
  "venue": "ICLR",
  "doi": "10.1234/example",
  "source_path": "data/documents/collection/example.pdf",
  "page_range": "3-4",
  "section": "Methods",
  "chunk_type": "body",
  "chunk_type_allowed": ["abstract", "body", "caption", "references"],
  "chunk_index": 3,
  "hash": "sha256-of-chunk",
  "image_refs": ["dochash_3_img1"],
  "is_reference": false
}
```

7.7 检索加权与排序策略
- 在融合/排序阶段引入 metadata boost：
  - `chunk_type=abstract`、`chunk_type=conclusion` +boost。
  - `is_reference=true` 降权或默认排除。
  - 对按 `year` 的软偏好：可把最近年份作为排序信号（soft boost）。
- 推荐融合公式示例：
  Score = α * dense_score + β * sparse_score + γ * metadata_boost
  其中 metadata_boost 由 chunk_type/recency/author_matching 等组合计算。

7.8 结果转换与引用（Provenance）
- 回答必须携带最小引用信息（作者+年份+chunk_index），并在需要时列出标准化参考条目（从 references 表中获取）。
- 若回答来自多个文献，返回去重后的来源列表并标注各自贡献片段。

7.9 去重与相似合并
- 入库去重：chunk-level content hash + semantic similarity（cos_sim > 0.95）去重，保留最完整/最新版本并记录来源列表。
- 对跨论文高度相似的“方法描述”可合并为 canonical snippet（可选高级功能），但需记录归属文献列表以便溯源。

7.10 实现建议与工具链
- 解析器：GROBID（首选）→ MarkItDown / PyMuPDF 回退。
- Embeddings：OpenAI / sentence-transformers（视预算）；摘要/结论优先编码并加权。
- Vector DB：Chroma（本地原型）/ Qdrant（扩展）。
- Reranker：Cross-Encoder 或 LLM Rerank；同时在 rerank 阶段使用 metadata 作为强信号。

7.11 建议文档改动位置
- 在 `other_DEV_SPEC.md` 中建议新增一节（如本节）并在 `3. 技术选型`、`4. 测试方案`、`5. 系统架构` 中交叉引用：
  - 将 Chunk 类型与 metadata schema 写入 3.1.1 的 Ingestion 描述中
  - 在 Retrieval 的 Filtering / Rerank 中明确加入 metadata boosts
  - 在测试用例中增加针对摘要优先、参考文献排除与题目匹配的回归测试

7.12 实现任务拆解（可直接排期）

### T1：文献结构解析增强（Parser + Loader）
- **目标**：提取 `title/authors/abstract/sections/references`，统一写入 Document metadata。
- **前置依赖**：
  - C1（核心类型定义已稳定）
  - C3（Loader 基础能力可用）
- **修改文件**：
  - `src/rag_mcp/parser/paper_parser.py`
  - `src/rag_mcp/ingestion/loader.py`
  - `src/rag_mcp/core/types.py`
- **实现类/函数**：
  - `extract_paper_structure(text_or_markdown) -> dict`
  - `normalize_paper_metadata(raw_meta) -> dict`
- **内部流程**：
  - 输入文本标准化（去 BOM、统一换行、清理多余空白）
  - 标题识别（优先 H1/首行高置信规则）
  - 作者识别（标题后若干行 + 机构/邮箱噪声剔除）
  - 摘要识别（`Abstract` 到下一章节标题）
  - 章节识别（`Introduction/Method/Experiment/Conclusion` 等）
  - 参考文献识别（`References/Bibliography` 段落拆分为条目）
  - 元数据归一化并写回 Document metadata
- **验收标准**：
  - 给定样本文献，`title` 与 `abstract` 字段均非空。
  - `references` 被识别为独立字段，且可统计条目数。
  - `sections` 至少包含 3 个一级结构（如 intro/method/conclusion）。
- **测试方法**：`pytest -q tests/unit/test_paper_parser_structure.py`

### T2：文献专用切分器（Smart Splitter 扩展）
- **目标**：新增按结构切分策略：abstract 单块、section 分块、references 特殊分块。
- **前置依赖**：
  - T1（结构字段已写入 metadata）
  - B3（Splitter 抽象层可扩展）
- **修改文件**：
  - `src/rag_mcp/splitter/paper_smart_splitter.py`
  - `src/rag_mcp/ingestion/pipeline.py`
- **实现类/函数**：
  - `split_paper_document(document) -> list[Chunk]`
  - `tag_chunk_type(chunk) -> str`
- **内部流程**：
  - 从 metadata 读取结构区块（abstract/sections/references）
  - 先产出 abstract chunk（强制单块）
  - 再按 section 进行长度约束切分（超长按段落/句子二次切）
  - references 区域单独切分并打 `is_reference=true`
  - 统一补齐 `chunk_type/section/chunk_index`
- **验收标准**：
  - `abstract` 仅生成 1 个 chunk。
  - `references` 相关 chunk 具备 `is_reference=true`。
  - 每个 chunk 均包含 `doc_id/chunk_type/chunk_index/section`。
- **测试方法**：`pytest -q tests/unit/test_paper_smart_splitter.py`

### T3：存储 schema 与写入策略
- **目标**：统一 chunk metadata 字段并在 upsert 前做 schema 校验。
- **前置依赖**：
  - T2（chunk_type 已稳定写入）
  - C12（VectorUpserter 幂等语义可用）
- **修改文件**：
  - `src/rag_mcp/vector_store/base.py`
  - `src/rag_mcp/vector_store/chroma_store.py`
  - `src/rag_mcp/core/types.py`
- **实现类/函数**：
  - `validate_chunk_metadata(metadata) -> None`
  - `build_chunk_record(chunk) -> dict`
- **内部流程**：
  - 定义必填字段集合（`doc_id/title/chunk_type/chunk_index`）
  - 写入前执行 metadata 校验并抛出字段级错误
  - 对枚举字段（`chunk_type`）执行合法性检查
  - 组装统一 record 后执行 upsert
- **验收标准**：
  - 缺失关键字段（`doc_id/chunk_type/title`）时抛出可读异常。
  - 向量库写入后可按 `chunk_type` 与 `year` 做过滤查询。
- **测试方法**：`pytest -q tests/unit/test_chunk_metadata_schema.py`

### T4：检索加权与过滤策略
- **目标**：在 hybrid + rerank 阶段加入 metadata boost 与 reference 降权。
- **前置依赖**：
  - D5（HybridSearch 基础流程可用）
  - D6（Reranker fallback 机制已完成）
- **修改文件**：
  - `src/rag_mcp/retrieval/hybrid_retriever.py`
  - `src/rag_mcp/retrieval/rrf.py`
  - `src/rag_mcp/reranker/llm_rerank.py`
- **实现类/函数**：
  - `compute_metadata_boost(result, query) -> float`
  - `apply_reference_penalty(results) -> list`
- **内部流程**：
  - dense/sparse 融合后计算 metadata_boost
  - abstract/conclusion 增益，reference 默认惩罚
  - 若 query 命中引用意图，则降低或取消 reference 惩罚
  - 输出最终排序并进入 rerank
- **验收标准**：
  - 同一 query 下，`abstract` chunk 平均排名高于普通 body chunk。
  - 未显式查询 references 时，reference chunk 不进入 Top-3。
- **测试方法**：`pytest -q tests/integration/test_paper_retrieval_ranking.py`

### T5：参考文献降权与按需召回
- **目标**：默认降权 references，仅在显式引用查询时放开。
- **前置依赖**：
  - T4（排序逻辑支持 reference penalty）
- **修改文件**：
  - `src/rag_mcp/retrieval/hybrid_retriever.py`
  - `src/rag_mcp/mcp/tools.py`
- **实现类/函数**：
  - `is_reference_intent(query) -> bool`
  - `filter_or_downweight_references(results, query) -> list`
- **内部流程**：
  - 解析 query 是否包含引用意图词（如“参考文献/引用/cite/bibliography”）
  - 非引用意图：执行 reference 降权/截断策略
  - 引用意图：放开 references，并保留较高召回比例
  - MCP tool 层根据意图切换返回策略
- **验收标准**：
  - 普通问答 query 下，references 结果占比 < 20%。
  - “参考文献/引用有哪些”类 query 下，references 可进入 Top-K。
- **测试方法**：`pytest -q tests/unit/test_reference_intent_and_penalty.py`

### T6：去重与合并
- **目标**：新增 hash + semantic 双重去重，控制重复入库。
- **前置依赖**：
  - C2（文件级 hash 能力可用）
  - C8（embedding 可用，支持相似度计算）
- **修改文件**：
  - `src/rag_mcp/ingestion/dedup.py`
  - `src/rag_mcp/ingestion/pipeline.py`
- **实现类/函数**：
  - `dedup_by_hash(chunks) -> list`
  - `dedup_by_similarity(chunks, threshold=0.95) -> list`
- **内部流程**：
  - 第一步：按文本 hash 去除完全重复 chunk
  - 第二步：对剩余 chunk 计算向量相似度，过滤近重复
  - 第三步：保留代表 chunk，并记录被合并来源映射
  - 第四步：输出去重后的 chunks 给 upsert
- **验收标准**：
  - 完全重复段落不会重复入库。
  - 近重复（sim > 0.95）被合并，且保留 1 条代表记录。
  - 非重复内容不误删（保留率 > 95%）。
- **测试方法**：`pytest -q tests/unit/test_dedup_hash_and_similarity.py`

7.13 对应测试用例矩阵（pytest）

建议新增测试文件
- `tests/unit/test_paper_parser_structure.py`
- `tests/unit/test_paper_smart_splitter.py`
- `tests/unit/test_chunk_metadata_schema.py`
- `tests/unit/test_reference_intent_and_penalty.py`
- `tests/unit/test_dedup_hash_and_similarity.py`
- `tests/integration/test_paper_ingestion_pipeline.py`
- `tests/integration/test_paper_retrieval_ranking.py`

核心测试用例

Case A: 题目/摘要提取
- Given：含标准论文标题与摘要的 markdown fixture。
- When：调用 `extract_paper_structure`。
- Then：`title` 精确匹配，`abstract` 非空且长度 > 100。

Case B: 文献结构切分
- Given：包含 `Abstract/Introduction/References` 的文档。
- When：调用 `split_paper_document`。
- Then：
  - 仅 1 个 `chunk_type=abstract`
  - 至少 1 个 `chunk_type=references`
  - 每个 chunk 含 `doc_id/chunk_index/section/chunk_type`

Case C: metadata schema 校验
- Given：缺失 `title` 的 chunk metadata。
- When：调用 `validate_chunk_metadata`。
- Then：抛出 `ValueError` 且错误信息包含缺失字段名。

Case D: references 降权
- Given：同一 query 的候选结果中含 body 与 references。
- When：执行 `apply_reference_penalty`。
- Then：references 平均分下降，Top-3 默认不含 references。

Case E: 引用意图放开
- Given：query=“这篇论文引用了哪些工作？”
- When：执行 `filter_or_downweight_references`。
- Then：references 不再被强过滤，Top-K 中至少出现 1 条。

Case F: 去重行为
- Given：10 个 chunk，其中 3 个完全重复，2 个近似重复（sim>0.95）。
- When：先 `dedup_by_hash` 再 `dedup_by_similarity`。
- Then：最终 chunk 数减少且保留一份代表性 chunk，并记录来源映射。

Case G: 端到端摄取回归
- Given：`tests/fixtures/papers/sample_paper.pdf`。
- When：跑完整 ingestion pipeline。
- Then：
  - 成功入库
  - `abstract` chunk 存在
  - `references` chunk 存在且标记正确
  - metadata 字段完整

Case H: 检索排序回归
- Given：与摘要强相关的 query。
- When：执行 hybrid retrieval + rerank。
- Then：Top-1 或 Top-2 命中 `chunk_type=abstract` 或 `conclusion`。

7.14 验证命令（建议）

```powershell
pytest -q tests/unit/test_paper_parser_structure.py
pytest -q tests/unit/test_paper_smart_splitter.py
pytest -q tests/unit/test_chunk_metadata_schema.py
pytest -q tests/unit/test_reference_intent_and_penalty.py
pytest -q tests/unit/test_dedup_hash_and_similarity.py
pytest -q tests/integration/test_paper_ingestion_pipeline.py
pytest -q tests/integration/test_paper_retrieval_ranking.py
```

---

（End of paper-specific adaptation draft）
