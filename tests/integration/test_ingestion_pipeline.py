    ...
    def test_pipeline_with_complex_technical_doc(self, settings, complex_pdf_path):
        ...
        try:
            ...
            assert encoding["dense_dimension"] == settings.embedding.dimensions, \
                f"Expected {settings.embedding.dimensions}-dim vectors for {settings.embedding.model}"
            ...
            bm25_dir = get_bm25_index_dir(collection, settings)
...
class TestPipelineComponents:
    ...
    def test_settings_loads_correctly(self, settings):
        """Verify settings are loaded with expected structure (provider-agnostic)."""
        # LLM settings
        assert settings.llm.provider, "LLM provider must be configured"
        assert settings.llm.model, "LLM model must be configured"
        print(f"[OK] LLM: {settings.llm.provider}/{settings.llm.model}")

        # Embedding settings
        assert settings.embedding.provider, "Embedding provider must be configured"
        assert settings.embedding.model, "Embedding model must be configured"
        assert settings.embedding.dimensions > 0, "Embedding dimensions must be > 0"
        print(f"[OK] Embedding: {settings.embedding.provider}/{settings.embedding.model} ({settings.embedding.dimensions}dim)")

        # Vision LLM settings
        assert settings.vision_llm is not None
        assert settings.vision_llm.enabled in (True, False)
        assert settings.vision_llm.provider, "Vision LLM provider must be configured"
        ...
        print(f"[OK] Ingestion LLM enhancement: chunk_refiner={settings.ingestion.chunk_refiner.get('use_llm')}, enricher={settings.ingestion.metadata_enricher.get('use_llm')}")
    
    def test_embedding_creates_vectors(self, settings):
        """Test that embedding service works with configured provider."""
        ...
        assert len(vectors[0]) == expected_dim, \
            f"Expected {expected_dim}-dim vectors for {settings.embedding.model}"
