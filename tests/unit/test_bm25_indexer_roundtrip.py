...
class TestBM25IndexerBasics:
    ...
    def test_indexer_initialization_requires_index_dir(self, tmp_path):
        """Test initialization with explicit runtime directory."""
        indexer = BM25Indexer(index_dir=str(tmp_path))
        
        assert indexer.k1 == 1.5
        assert indexer.b == 0.75
        assert os.path.normpath(str(indexer.index_dir)) == os.path.normpath(str(tmp_path))
...
class TestBM25Querying:
    ...
    def test_query_before_build_raises(self):
        ...
        indexer = BM25Indexer(index_dir="test_runtime/bm25")
...
class TestRebuildFunctionality:
    ...
    def test_rebuild_merges_old_index(self, tmp_path):
        """Test that rebuild merges on top of existing index."""
        indexer = BM25Indexer(index_dir=str(tmp_path))
        ...
        # Old term should still exist under merge semantics
    ...
    def test_build_merges_existing_index_idempotent(self, tmp_path):
        """Test build merges on top of existing index without duplication."""
        indexer = BM25Indexer(index_dir=str(tmp_path))

        term_stats1 = [
            {"chunk_id": "doc1", "term_frequencies": {"alpha": 2}, "doc_length": 2}
        ]
        term_stats2 = [
            {"chunk_id": "doc2", "term_frequencies": {"beta": 1}, "doc_length": 1}
        ]

        indexer.build(term_stats1, collection="test")
        indexer.build(term_stats2, collection="test")

        results_alpha = indexer.query(["alpha"], top_k=10)
        results_beta = indexer.query(["beta"], top_k=10)

        assert len(results_alpha) == 1
        assert results_alpha[0]["chunk_id"] == "doc1"
        assert len(results_beta) == 1
        assert results_beta[0]["chunk_id"] == "doc2"

        df_alpha_before = indexer._index["alpha"]["df"]
        df_beta_before = indexer._index["beta"]["df"]

        indexer.build(term_stats2, collection="test")

        assert indexer._index["alpha"]["df"] == df_alpha_before
        assert indexer._index["beta"]["df"] == df_beta_before
