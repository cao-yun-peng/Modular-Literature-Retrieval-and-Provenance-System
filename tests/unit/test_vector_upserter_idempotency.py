...
def mock_settings():
    ...
    settings.vector_store.persist_directory = "C:/test_runtime/chroma"
    return settings
...
def test_chunk_id_format(upserter_with_mock_store, sample_chunk):
    """Test that _generate_chunk_id returns chunk.id (ID generation delegated upstream)."""
    upserter, _ = upserter_with_mock_store

    chunk_id = upserter._generate_chunk_id(sample_chunk)

    # IDs now come directly from Chunk.id (set upstream by splitter/loader)
    assert chunk_id == sample_chunk.id, "ID must match chunk.id"
    assert isinstance(chunk_id, str)


def test_chunk_id_changes_with_content(upserter_with_mock_store, sample_chunk):
    """Test that _generate_chunk_id returns chunk.id regardless of content changes."""
    upserter, _ = upserter_with_mock_store

    original_id = sample_chunk.id
    id1 = upserter._generate_chunk_id(sample_chunk)

    # Modify content — ID comes from chunk.id, not content hash
    sample_chunk.text = "Different content"
    id2 = upserter._generate_chunk_id(sample_chunk)

    assert id1 == id2 == original_id, "ID depends on chunk.id, not content"


def test_chunk_id_changes_with_index(upserter_with_mock_store, sample_chunk):
    """Test that _generate_chunk_id returns chunk.id regardless of index changes."""
    upserter, _ = upserter_with_mock_store

    original_id = sample_chunk.id
    id1 = upserter._generate_chunk_id(sample_chunk)

    # Modify index — ID comes from chunk.id, not metadata
    sample_chunk.metadata["chunk_index"] = 5
    id2 = upserter._generate_chunk_id(sample_chunk)

    assert id1 == id2 == original_id, "ID depends on chunk.id, not chunk_index metadata"


def test_chunk_id_changes_with_source_path(upserter_with_mock_store, sample_chunk):
    """Test that _generate_chunk_id returns chunk.id regardless of source_path changes."""
    upserter, _ = upserter_with_mock_store

    original_id = sample_chunk.id
    id1 = upserter._generate_chunk_id(sample_chunk)

    # Modify source path — ID comes from chunk.id, not metadata
    sample_chunk.metadata["source_path"] = "data/documents/other.pdf"
    id2 = upserter._generate_chunk_id(sample_chunk)

    assert id1 == id2 == original_id, "ID depends on chunk.id, not source_path metadata"
...
def test_chunk_id_generation_missing_chunk_index(upserter_with_mock_store):
    """Test that _generate_chunk_id works without chunk_index (no validation in deprecated method)."""
    upserter, _ = upserter_with_mock_store
    ...
    assert chunk_id == "temp", "ID comes from chunk.id, no chunk_index validation"
...
def test_chunk_with_large_index(upserter_with_mock_store):
    """Test that _generate_chunk_id returns chunk.id regardless of index value."""
    upserter, _ = upserter_with_mock_store
    ...
    # ID comes from chunk.id, not formatted from metadata
    assert chunk_id == "temp"
