...
class TestConfigProperty:
    ...
    def test_config_derived_from_settings(self, mock_settings: Mock):
        ...
        mock_settings.vector_store.persist_directory = "C:/settings/path"
        mock_settings.vector_store.collection_name = "settings_collection"
        
        tool = GetDocumentSummaryTool(settings=mock_settings)
        config = tool.config
        
        assert Path(config.persist_directory).as_posix() == "C:/settings/path"
        assert config.default_collection == "settings_collection"

    def test_config_derived_from_mock_settings_fixture(self, mock_settings: Mock):
        """Test that config uses configured vector store path from settings."""
        tool = GetDocumentSummaryTool(settings=mock_settings)
        config = tool.config

        assert config.persist_directory == str(get_vector_store_persist_dir(mock_settings))
