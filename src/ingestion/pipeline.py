        ...
        try:
            ...
            if trace is not None:
                ...
                trace.record_stage("upsert", {
                    "method": "chromadb+bm25+images",
                    "dense_store": {
                        "backend": "ChromaDB",
                        "collection": self.collection,
                        "count": len(vector_ids),
                        "path": str(resolve_path(self.settings.vector_store.persist_directory)),
                    },
                    "sparse_store": {
                        "backend": "BM25",
                        "collection": self.collection,
                        "count": len(sparse_stats),
                        "path": str(get_bm25_index_dir(self.collection, self.settings)),
                    },
                    "image_store": {
...
def run_pipeline(
    ...
    pipeline = IngestionPipeline(
        settings, collection=collection, force=force,
        use_paper_loader=use_paper_loader,
    )
