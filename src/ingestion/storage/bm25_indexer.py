...
- Persistent: Indexes saved to the configured runtime BM25 directory
...
class BM25Indexer:
    ...
    Example:
        >>> indexer = BM25Indexer(index_dir="/runtime/bm25")
        >>> 
    ...
    def __init__(
        ...
        Args:
            index_dir: Directory to store index files. If omitted, uses a
                temp-directory runtime path.
        ...
        if index_dir is None:
            index_dir = str(Path(tempfile.gettempdir()) / "modular_rag_bm25")
    ...
    def build(
        ...
        # If an index already exists, rebuild on top of it (idempotent merge)
        if not self._index:
            load_ok = self.load(collection)
            logger.info(
                "BM25 build: collection=%s index_path=%s load_ok=%s",
                collection,
        ...
        if self._index:
            existing_stats = self._reconstruct_term_stats_from_index()
            if existing_stats:
                before_count = len(existing_stats)
                incoming_ids = {stat["chunk_id"] for stat in term_stats}
                # Remove any existing stats for incoming chunk_ids to keep idempotency
                existing_stats = [
                    stat for stat in existing_stats
                    if stat["chunk_id"] not in incoming_ids
                ]
                term_stats = existing_stats + term_stats
                after_count = len(term_stats)
                logger.info(
                    "BM25 build merge: collection=%s chunks_before=%s chunks_after=%s",
                    collection,
                    before_count,
                    after_count,
                )
    ...
    def _reconstruct_term_stats_from_index(self) -> List[Dict[str, Any]]:
        """Reconstruct term_stats from the current in-memory index.

        Returns:
            List of term_stats dicts suitable for build().
        """
        stats_by_chunk: Dict[str, Dict[str, Any]] = {}

        for term, term_data in self._index.items():
            for posting in term_data.get("postings", []):
                chunk_id = posting["chunk_id"]
                entry = stats_by_chunk.get(chunk_id)
                if entry is None:
                    entry = {
                        "chunk_id": chunk_id,
                        "term_frequencies": {},
                    ...
                    stats_by_chunk[chunk_id] = entry

                entry["term_frequencies"][term] = posting["tf"]

        return list(stats_by_chunk.values())
    ...
    def _save(self, collection: str) -> None:
        ...
        try:
            # Write directly to avoid Windows rename/locking issues on
            # constrained workspaces. This is sufficient for test/runtime use.
