...
- Reusing Chunk.id to keep IDs consistent across stores
...
class VectorUpserter:
    ...
    reuses Chunk.id, and writes them to the configured vector store.
    
    Chunk ID Format:
        {doc_id}_{chunk_index:04d}_{content_hash}

        Where:
        - doc_id = Document ID produced by loaders
    ...
    This ensures:
        ...
        - Consistent IDs across dense and sparse stores
    ...
    Example:
        ...
        >>> # Chunks written with stable IDs like: "doc_123_0000_e5f6g7h8"
    """
    ...
    def upsert(
        ...
        # Reuse Chunk.id to keep IDs consistent across sparse and dense stores
        records = []
        chunk_ids = []
    ...
    def _generate_chunk_id(self, chunk: Chunk) -> str:
        """Deprecated: IDs now come directly from Chunk.id."""
        return chunk.id
