...
class ChunkRefiner(BaseTransform):
    ...
    def __init__(
        ...
        self._llm_configured = self.use_llm  # Preserve original config intent
    ...
    def _refine_single_chunk(
        ...
        try:
            ...
            meta = {**(chunk.metadata or {}), 'refined_by': refined_by}
            if fallback_reason:
                meta['refine_fallback_reason'] = fallback_reason
            refined_chunk = Chunk(
                id=chunk.id,
                text=refined_text,
                metadata=meta,
        ...
        for chunk in chunks:
            try:
                ...
                if self.use_llm and self.llm:
                    ...
                    # LLM disabled or init failed, use rule-based
                    refined_text = rule_refined_text
                    refined_by = "rule"
                    if self._llm_configured:
                        # LLM was enabled but could not be used (init failed)
                        if chunk.metadata is not None:
                            chunk.metadata['refine_fallback_reason'] = "llm_init_failed"
