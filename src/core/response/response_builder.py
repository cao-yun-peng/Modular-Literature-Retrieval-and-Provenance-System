...
from src.core.response.table_content import TableContent
...
class MCPToolResponse:
    ...
    def to_mcp_content(self) -> List[Union[types.TextContent, types.ImageContent]]:
        ...
        for table in self.table_contents:
            blocks.append(table.to_mcp_content())
...
class ResponseBuilder:
    ...
    def build(
        ...
        Args:
            ...
            linked_assets: Optional mapping of ``chunk_id`` → linked figure/table
                data produced by ``LinkedAssetResolver``.
        ...
        content = self._build_markdown_content(results, citations, query, linked_assets)
        ...
        table_contents = self._extract_table_contents(results)
        if table_contents:
            metadata["has_tables"] = True
            metadata["table_count"] = len(table_contents)

        if linked_assets:
            metadata["has_linked_assets"] = True
            metadata["linked_asset_count"] = len(linked_assets)
    ...
    def _build_markdown_content(
        ...
        """Build Markdown content with inline citations and linked assets.

        Args:
            results: List of RetrievalResult.
            citations: List of Citation objects.
            query: Original query string.
            linked_assets: Optional mapping of chunk_id → linked figures/tables.
        ...
        for i, (result, citation) in enumerate(zip(results[:display_count], citations[:display_count])):
            ...
            # Append linked figures/tables if present
            if linked_assets and result.chunk_id in linked_assets:
                assets = linked_assets[result.chunk_id]
                linked_lines = self._format_linked_assets(assets)
    ...
    def _extract_table_contents(
        self, results: List[RetrievalResult]
    ) -> List[TableContent]:
        tables: List[TableContent] = []
        for result in results:
            metadata = result.metadata or {}
            table_list = metadata.get("tables") or []
            ...
            for table in table_list:
                ...
                if table_id and headers is not None and rows is not None:
                    tables.append(
                        TableContent(
    ...
    def _format_linked_assets(self, assets: Dict[str, Any]) -> List[str]:
        """Format linked figure/table assets as markdown lines.

        Args:
            assets: Dict with ``"figures"`` and ``"tables"`` lists.

        Returns:
            List of markdown lines to append after a result snippet.
        """
        lines: List[str] = []
        ...
        summary_parts = []
        if figures:
            summary_parts.append(f"{len(figures)} figures")
        if tables:
            summary_parts.append(f"{len(tables)} tables")
        lines.append(f"<summary>📊 关联图表 ({', '.join(summary_parts)})</summary>")
        lines.append("")
        ...
        for fig in figures:
            ...
            text = fig.get("text", "")
            lines.append(f"**[FIG_REF: {fid}]** *{caption}*")
            # Extract description line (second line of figure chunk text)
            desc = text.split("\n", 1)
            if len(desc) > 1:
                lines.append(f"> {self._truncate_text(desc[1], 200)}")
            lines.append("")

        for tab in tables:
            tid = tab.get("table_id", "?")
            caption = tab.get("caption", "")
            text = tab.get("text", "")
            lines.append(f"**[TABLE_REF: {tid}]** *{caption}*")
            # Show table content (already tab-separated)
            content_lines = text.split("\n")
            # Skip the [TABLE_DATA: ...] and Caption: lines, show Content:
            in_content = False
            table_body: List[str] = []
            for cl in content_lines:
                if cl.startswith("Content:"):
                    in_content = True
                    continue
                if in_content and cl.strip():
                    table_body.append(cl.strip())
            if table_body:
                for row in table_body[:8]:  # max 8 rows
                    lines.append(f"> `{row}`")
                if len(table_body) > 8:
                    lines.append(f"> ... *and {len(table_body) - 8} more rows*")
            lines.append("")
