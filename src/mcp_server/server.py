...
async def run_stdio_server_async() -> int:
    ...
    # Warm up heavy native-extension imports as early as possible.
    # On Windows, importing scientific stack modules (e.g., numpy) after
    # background threads/event loop machinery is active can intermittently hang
    # during native module loading. Pre-importing at process startup avoids that
    # failure mode for tools that depend on these libraries.
    if str(os.getenv("MODULAR_RAG_SKIP_IMPORT_WARMUP", "")).lower() not in {"1", "true", "yes"}:
        try:
            import numpy  # noqa: F401
            logger.info("Warm-up import OK: numpy")
        except Exception as exc:
            logger.warning("Warm-up import failed: numpy (%r)", exc)

        try:
            import chromadb  # noqa: F401
            logger.info("Warm-up import OK: chromadb")
        except Exception as exc:
            logger.warning("Warm-up import failed: chromadb (%r)", exc)
