"""TraceService – read and parse traces from the configured trace JSONL file.
...
from src.core.settings import get_trace_file_path
...
class TraceService:
    ...
    Args:
        traces_path: Path to the JSONL file.  Defaults to
            the path configured in ``settings.yaml``.
    """

    def __init__(self, traces_path: Optional[str | Path] = None) -> None:
        self.traces_path = Path(traces_path) if traces_path else get_trace_file_path()
