...
def _render_load_stage(data: Dict[str, Any], trace_id) -> None:
    ...
    if preview:
        ...
        st.text_area(
            ...
            key=f"raw_text_{trace_id}",
        )
    else:
        st.info("No text preview recorded in this trace.")


def _render_split_stage(data: Dict[str, Any], trace_id) -> None:
        ...
        for i, chunk in enumerate(chunks):
            ...
            with st.expander(header, expanded=(i < 2)):
                st.text_area(
                    ...
                    disabled=True,
                    label_visibility="collapsed",
                    key=f"raw_text_{trace_id}_{i}",
                )
    else:
        st.info("No chunk text recorded. Re-run ingestion to generate new traces.")


def _render_transform_stage(data: Dict[str, Any], trace_id) -> None:
                ...
                if text_before or text_after:
                    ...
                    with col_before:
                        ...
                        st.text_area(
                            ...
                            disabled=True,
                            label_visibility="collapsed",
                            key=f"raw_text_{trace_id}_before_{i}",
                        )
                    with col_after:
                        ...
                        st.text_area(
                            ...
                            disabled=True,
                            label_visibility="collapsed",
                            key=f"raw_text_{trace_id}_after_{i}",
                        )
    else:
