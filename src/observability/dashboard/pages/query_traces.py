                ...
                for tab, (label, key) in zip(tabs, tab_defs):
                    with tab:
                        ...
                        if key == "query_processing":
                            ...
                            _render_retrieval_stage(data, "Dense", trace_id)
                        elif key == "sparse_retrieval":
                            _render_retrieval_stage(data, "Sparse", trace_id)
                        elif key == "fusion":
                            _render_fusion_stage(data, trace_id)
                        elif key == "rerank":
                            _render_rerank_stage(data, trace_id)
            else:
...
def _render_retrieval_stage(data: Dict[str, Any], label: str, trace_id: str) -> None:
    ...
    chunks = data.get("chunks", [])
    if chunks:
        _render_chunk_list(chunks, trace_id, prefix=f"{label.lower().replace(' ', '_')}_chunk")
    else:
        st.info(f"No {label.lower()} results returned.")


def _render_fusion_stage(data: Dict[str, Any], trace_id) -> None:
    ...
    chunks = data.get("chunks", [])
    if chunks:
        _render_chunk_list(chunks, trace_id, prefix="fusion_chunk")
    else:
        st.info("No fusion results.")


def _render_rerank_stage(data: Dict[str, Any], trace_id) -> None:
    ...
    chunks = data.get("chunks", [])
    if chunks:
        _render_chunk_list(chunks, trace_id, prefix="rerank_chunk")
    else:
        st.info("No reranked results.")


def _render_chunk_list(chunks: List[Dict[str, Any]], trace_id, prefix: str = "chunk") -> None:
        ...
        with st.expander(header, expanded=False):
            ...
            if text:
                st.text_area(
                    ...
                    key=f"split_text_{trace_id}_{prefix}_{ci}"
                )
            else:
