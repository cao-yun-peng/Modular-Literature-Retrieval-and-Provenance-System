...
class ChromaStore(BaseVectorStore):
    ...
    def __init__(self, settings: Settings, **kwargs: Any) -> None:
        ...
        persist_dir_str = kwargs.get(
            'persist_directory',
            getattr(vector_store_config, 'persist_directory', None)
        )
        if not persist_dir_str:
            raise ValueError(
                "Missing required vector store persist_directory in settings.vector_store"
            )
    ...
    def _build_where_clause(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # Chroma expects a single top-level operator for compound filters.
        # For multi-field exact matches, wrap them in `$and`.
    ...
    def get_by_metadata(
        ...
        """Retrieve records matching metadata filters (exact match).

        Used by the retrieval layer to look up linked figure/table chunks
        by their ``figure_id`` or ``table_id`` metadata fields.

        Args:
            filters: Metadata key/value pairs to match.

        Returns:
            List of matching records with ``id``, ``text``, ``metadata`` keys.
        ...
        try:
            ...
            logger.warning("get_by_metadata failed for filters=%s: %s", filters, e)
            return []
    ...
    def close(self) -> None:
        ...
        On Windows, ChromaDB's SQLite backend holds exclusive file locks
        that prevent cleanup of the persist directory until the client is
        properly closed. Call this before deleting the persist directory.
        """
        try:
